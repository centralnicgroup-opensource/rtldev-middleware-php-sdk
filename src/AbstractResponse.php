 ...
 * assembly of the {@see Paginator} — and leaves the parts that genuinely differ
 ...
 *     own metadata says, and every derivation from those four answers is shared
 *     and written once — on {@see Paginator} since RSRMID-2965, which this base
 *     only assembles (see {@see getPagination()}). RSRMID-2943 first collected
 *     those derivations here, for the same reason they now sit one step further
 *     out: none of them reads a column of its own.
 *
...
abstract class AbstractResponse implements ResponseInterface
{
     ...
     *
     * The one place the four brand primitives meet the shared arithmetic. Every
     * derivation from them — page numbers, the page count, the has-next/
     * has-previous predicates — lives on {@see Paginator} since RSRMID-2965,
     * because none of it reads a column, holds state or needs a wire payload:
     * keeping it here meant an offset grid could only be exercised by
     * hand-authoring an API response that carried four integers.
     *
     * A fresh Paginator per call, over numbers that can no longer change (a
     * response is sealed once constructed), so two callers cannot observe each
     * other and there is no cache to invalidate.
     *
     * {@see getRecordsCount()} supplies the fifth member deliberately: it counts
     * the rows this response holds and is {@see getRecord()}'s bounds authority,
     * so it is the reading that cannot be made to lie by a wire that miscounts.
     */
    #[\Override]
