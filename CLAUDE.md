...
- Do **not** re-add `setRoleCredentials()` to `AbstractClient` — it is CNR-only (depends on the CNR role separator; inheriting it on IBS/Moniker would forge a garbage login) and lives on `RoleCredentialsInterface` (CNR only); narrow via `instanceof \CNIC\RoleCredentialsInterface` to use it. This is the client-tree twin of the `ExtendedResponseInterface` rule above. `useHighPerformanceConnectionSetup()` deliberately **stays** on `AbstractClient` (brand-agnostic loopback routing). (Ref: RSRMID-2911; note that the same ticket's decision to keep `getSession()`/`setSession()` there was reversed — see the next bullet.)
- Do **not** put the session/persistent/role-separator state back on the shared config or client. Since RSRMID-2920 (breaking, v22.0.0) `$session`/`$persistent`/`$roleSeparator` and their accessors (`getSession`/`setSession`/`getPersistent`/`setPersistent`/`getRoleSeparator`) exist **only** on `CNR\SocketConfig`; `getSession()`/`setSession()` on the client live **only** on `CNR\Client`; `persistent=1` is emitted by CNR's `getPOSTDataParams()`, not the shared `getPOSTData()`; and there is no `IBS\SessionClient`/`MONIKER\SessionClient` (the factory returns the brand `Client`). Do **not** make `MONIKER\Client` `final` to satisfy Psalm's `ClassMustBeFinal` — that rule only fires on leaf classes, so `CNR\Client`/`IBS\Client` escape it merely because something internal extends them; the brand clients are extensible as a set, and the issue is suppressed at the class with that rationale. `CNR\Client::getSocketConfig()` (the covariant override, formerly the protected `cnrConfig()` — renamed in RSRMID-2921) is the **one** narrowing point from the invariant `AbstractSocketConfig` property type — go through it, do not add per-call asserts. **The policy this settled:** for a capability a platform cannot honour, prefer **absence** (a call-site type error) where the type system can express it; where the method must exist on a shared surface, **throw** (RSRMID-2919); **never no-op** — the fluent no-ops this replaced made `ibs()->setSession("x")` look accepted for six majors. Guarded by `tests/ClientSessionSeamTest.php` (reflection is the only instrument that can see absence eroding). Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
- Do **not** reimplement the `request()` lifecycle per brand — it is a template method on `AbstractClient::performRequest()`. The public `request(array $cmd = [], string $path = "")` signature is **symmetric across all brands** (CNR just defaults `$path` to `api/call.cgi`); vary a brand only through the `buildCommand()`/`newResponse()` hooks and its `newSocketConfig()` subtype (where the cURL option bag lives since RSRMID-2921) — though as of RSRMID-2915 **no brand varies the bag**, see the next bullet. (Ref: RSRMID-2909, which deliberately dropped the earlier "CNR must never accept a per-request path" rule.)
- Do **not** give connection configuration a second home. Since RSRMID-2921 (breaking, v23.0.0) `$url`/`$system`/`$curlopts`/`$proxy`/`$referer`/`$highPerformance` live **only** on `AbstractSocketConfig`; `AbstractClient` declares none of them and its ~26 configuration methods are forwarders (keep them — they are the documented consumer surface; what changed is that a _new_ setting needs no new forwarder, because `getSocketConfig()` exists). Two invariants are load-bearing: **the `System` is derived from the URL** (`getSystem(): ?System`, nullable on purpose — a custom URL has no honest OT&E-or-LIVE answer; there is no flag left to contradict the URL), and **a value the SDK models cannot also come from the cURL bag** (`AbstractSocketConfig::MANAGED_OPTIONS` — `CURLOPT_TIMEOUT`/`USERAGENT`/`PROXY`/`REFERER` — raises `UnsupportedFeatureException` naming the owning setter, eagerly at the setter, unlike the transport's own set which is checked per request). `useHighPerformanceConnectionSetup()` is a **flag applied inside `getURL()`**, not a URL rewrite — an eager rewrite silently cost the caller `isOTE()`. Do **not** re-add `cnrConfig()` as an alias, do **not** widen/narrow `MANAGED_OPTIONS` casually, and do **not** make `getUserAgent()` memoise again. Guarded by `tests/ClientConfigSeamTest.php` (structural — a re-added copy is behaviour-preserving the day it lands, so only reflection can refuse it) and `tests/AbstractClientConfigDriftTest.php` (the three drifts). Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
- Do **not** put CNR's IDN command rewriting back on the shared tree. Since RSRMID-2922 (breaking, v24.0.0) the rules live in `CNIC\CNR\IDNCommandRewriter::rewrite()` (a `final` class of static rules, same shape as `CommandFormatter`), called from `CNR\Client::buildCommand()` — the brand hook `performRequest()` already provides. `AbstractClient::autoIDNConvert()`, the `needsIDNConvert` property and `AbstractSocketConfig::getNeedsIDNConvert()` are **gone**; do not re-add them, and do not add a brand-detection branch to `performRequest()` in their place (the call site is the brand). The public `AbstractClient::IDNConvert()` deliberately **stays** — converting a caller-supplied list of names is brand-agnostic and public API since v7.1.0. The rewrite must run **after** `CommandFormatter::flattenCommand()` (the rules match wire keys such as `NAMESERVER0`/`OBJECTID`, not the caller's nested input) and must rewrite values in place so the priority sort survives. Guarded by `tests/CNR/IDNCommandRewriterTest.php` (the rules, via the public method — no reflection) and `tests/ClientIDNSeamTest.php` (structural: hoisting the rules back behind a flag is behaviour-preserving the day it lands, so only reflection can refuse it). Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
- Do **not** re-add a brand-default cURL option. `IBS\Client` used to force IPv4 (`CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4`) via `getDefaultCurlOpts()`; that shipped one customer's network workaround to every integration and was removed in RSRMID-2915 (breaking, v20.0.0). Transport tuning is the **caller's** call via `setExtraCurlOptions()`. The `getDefaultCurlOpts()` hook stays (it is what `resetCurlOptions()` restores) but moved to `AbstractSocketConfig` with the option bag in RSRMID-2921, and all three brand **configs** must resolve it there — asserted by `ClientConfigSeamTest::testNoBrandOverridesTheDefaultCurlOptsHook()`, with `testClientsHaveNoCurlOptionDefaultHook()` banning a second hook on the clients. A genuinely protocol-mandatory option could justify an override; papering over one environment's networking cannot. Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
- Do **not** make a caller's cURL option fail silently again, and do **not** widen or narrow `HttpTransport::PROTECTED_OPTIONS` casually. Since RSRMID-2919 (breaking, v21.0.0) `HttpTransport::post()` applies the caller's options **over** its own defaults (`$options + [defaults]`), so `setExtraCurlOptions()` reaches the wire; the closed set it still owns — `CURLOPT_URL`/`POST`/`POSTFIELDS`/`RETURNTRANSFER`/`HEADER` (request envelope) plus `SSL_VERIFYPEER`/`SSL_VERIFYHOST` (TLS posture) — raises `UnsupportedFeatureException` naming the constants rather than being ignored. `CURLOPT_HTTPHEADER` is additive — caller lines are appended, and restating one of the transport's own (`Expect:`/`Content-Type`/`Content-Length`/`Connection`) throws too, since those are derived from the POST body and the reused handle. The request timeout has a real setter, `AbstractClient::setSocketTimeout()`, which rejects negatives (cURL drops them silently) and since RSRMID-2921 is the **only** route — `CURLOPT_TIMEOUT` in the bag now raises, along with the other three `MANAGED_OPTIONS` (see the configuration-home rule below). Guarded by `HttpTransportCurlOptionsTest` (rejection, offline) and `tests/Functional/HttpTransportTest.php` (effective merge, loopback) — bag-only assertions cannot see this defect class, which is exactly how it survived two releases. Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
...
- Do **not** re-split the record/column layer per brand. Since RSRMID-2923 (breaking, v25.0.0) there is exactly one `CNIC\Record` (concrete; `AbstractRecord`, `CNR\Record` and `IBS\Record` are **gone** — the latter two were byte-identical empty subclasses) and one `CNIC\Column` (concrete, `@template TValue`, used directly by IBS/Moniker). `CNR\Column` survives **only** to bind `TValue` to `string` and narrow `getDataByIndex()` to `?string`; if you find yourself adding a second method there, the duplication is coming back. The empirical result the ticket asked for: that covariant narrowing against a `mixed` base return is **accepted by both** PHPStan L9 and Psalm L1, and the templated `@extends CNIC\Column<string>` binding is genuinely enforced (passing `int[]` to a CNR column is now an error where the old hand-rolled class needed a `@psalm-suppress MoreSpecificImplementedParamType` — that suppression is gone). `AbstractResponse::newRecord()` deliberately **stays** an abstract per-brand hook even though both brands now return the same `CNIC\Record`: it is the seam a brand with genuinely different row behaviour would use, and hard-coding the shared Record on the base would close it. Guarded by `tests/RecordColumnSeamTest.php` (structural — re-adding an empty brand marker or re-inlining the shared body is behaviour-preserving the day it lands, so only reflection can refuse it; both mutations were verified to fail it). Shared behaviour is covered once in `tests/ColumnTest.php`/`tests/RecordTest.php`; brand test classes cover only the `addColumn()`/`newRecord()` wiring and CNR's narrowing. Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
- Do **not** hard-wire the parse step back into `populate()`. Since RSRMID-2924 (breaking, v26.0.0) the brand parsers are **instance** classes implementing `CNIC\ResponseParserInterface` (`parse(string $raw, array $cmd = []): array` — one signature for both brands; CNR accepts `$cmd` and ignores it), reached through `$this->parser` on `AbstractResponse`, which is either the brand's own via the abstract `newResponseParser()` hook or the substitute passed as the constructor's optional 5th argument. Mirrors the `TransportInterface` seam (RSRMID-2910): factory hook for the default, injection point for the double. `AbstractResponseTemplateManager::parseResponse()` is **gone** — the manager uses the same `newResponseParser()` hook, and parses templates with **no** command on purpose (a template is not tied to one; both routes reach the same hash, which is now asserted rather than assumed). Do **not** make `parse()` static again, do **not** call `new RP()` inside `populate()` (behaviour-identical, silently closes the seam), and do **not** re-add a second parse route on the manager. Behind the interface the hash is `array<string, mixed>`, so `CNR\Response::stringCells()` re-establishes the brand's string-list shape for `PROPERTY` and **throws** `UnsupportedFeatureException` on a non-list entry **or** a non-string cell — do not soften either to a skip or an `(array)` cast (a cast coerces a bad container while the cell check refuses a bad cell, which is the inconsistency that was caught here). A missing or non-array `PROPERTY` block still yields no columns, deliberately. `AbstractResponseTemplateManager::resetTemplates()` (additive) restores a brand's built-in templates — `$templates` is public static state with process lifetime, so test classes that call `addTemplate()` reset it in `tearDownAfterClass()`; it is `addTemplate()`'s counterpart only, and a direct write to the public property escapes it (documented on the method, not assumed away). Guarded by `tests/ResponseParserSeamTest.php` (behavioural half: a substitute reaches the hash; structural half: only reflection can refuse a static re-wiring — verified, with the seam bypassed all 495 other tests stay green). Direct parser tests live in `tests/<Brand>/ResponseParserTest.php`; keep parse assertions out of `ResponseTest.php`. When asserting the IBS manager-vs-populate branch divergence, the command must **lack** `ResponseFormat` — one carrying `ResponseFormat=JSON` takes the same branch as the empty one and proves nothing. Rationale in [docs/agents/architecture.md](docs/agents/architecture.md).
...
- Do **not** declare `__construct()` on an interface. As of RSRMID-2923 **none of the three** carries one — `ResponseInterface` lost it in RSRMID-2918, `ColumnInterface`/`RecordInterface` in RSRMID-2923 (a relaxation, so non-breaking). The reason is design, not toothlessness: nothing constructs through these types — construction belongs to the brand factory hooks (`AbstractClient::newResponse()` / `AbstractResponseTemplateManager::createResponse()` / `AbstractResponse::newRecord()`) and to each brand's `addColumn()`, all of which name their own concrete class — so the declaration bought no caller anything while constraining every implementer to one construction shape (a column could not be backed by a generator or a cursor; a record could not wrap anything but a plain array). **Correction to the RSRMID-2918 rationale, which said the opposite and was wrong twice over:** PHP does **not** exempt an interface-declared constructor — an incompatible implementation is a declaration-time fatal (the exemption is class-to-class inheritance only), so the constraint had real teeth; and the `ResponseInterface` drift (3 params against `AbstractResponse`'s 4) survived not because nothing was checked but because the 4th, `$context`, is **optional**, and adding an optional parameter is legal widening. Both facts were verified by running them. Guarded by reflection in `tests/ResponseInterfaceConsumerTest.php` and `tests/RecordColumnSeamTest.php` — still the only possible guard, because a re-add is legal PHP as long as the implementations happen to match. (Ref: RSRMID-2918, corrected and extended in RSRMID-2923.)
- Do **not** give `AbstractResponse` single-page pagination defaults (nor a `SinglePageResponse` trait), and do **not** delete `IBS\Response`'s 7 primitive overrides (`getCurrentPageNumber`/`getFirstRecordIndex`/`getLastRecordIndex`/`getRecordsTotalCount`/`getRecordsLimitation`/`hasNextPage`/`hasPreviousPage`) as "redundant" — only the **derived** getters (`getNextPageNumber`/`getNumberOfPages`/`getPagination`/`getPreviousPageNumber`/`getRecordsCount`) belong on the base. A base default would turn "brand forgot pagination" into a silent "one page, no next page" that loses pages 2..N; rationale in [docs/agents/architecture.md](docs/agents/architecture.md). Guarded by `tests/ResponsePaginationSeamTest.php` — the erosion is behaviour-preserving, so that reflection test is the only thing that can catch it; deleting it means reopening the decision, which is recorded in [docs/agents/architecture.md](docs/agents/architecture.md) (raise a new issue against RSRMID-2918 — RSRMID-2912 is closed as Cancelled, the refactor having never been implemented). (Ref: RSRMID-2912 declined, RSRMID-2918.)
- `ClientFactory` exposes **typed named constructors** — `cnr(): CNR\SessionClient`, `ibs(): IBS\Client`, `moniker(): MONIKER\Client` — not a string-dispatch `getClient(string)`. CNR is the **only** brand with a `SessionClient`; the IBS/Moniker ones were empty and were deleted in RSRMID-2920. The returned client is fully typed, so brand-specific capabilities (CNR `login()`/`logout()`/`saveSession()`/`getSession()`/`setSession()`/`setRoleCredentials()`) are available directly, with no narrowing on the normal path. The old `Registrar` enum and `UnknownRegistrarException` were removed with `getClient()`. (Ref: RSRMID-2911, return types corrected in RSRMID-2920.)
