...
- **An interface declaration must match its implementation's signature.** Because consumers are told to type against the interfaces above, a parameter that exists only on the implementation is unreachable to them: the call runs, but PHPStan/Psalm reject it (`Method X invoked with 1 parameter, 0 required`), pushing consumers to retype against a concrete class or silence the analyser. When adding a parameter to an implemented method, add it to the interface in the same change — that is **breaking** for anyone implementing the interface, so it needs a `BREAKING CHANGE:` footer + `MIGRATION.md` entry. Guarded by `tests/ResponseInterfaceConsumerTest.php`, which makes the interface-typed calls that the analysers then enforce. (Ref: RSRMID-2918, which fixed `ResponseInterface::getColumnKeys()`. Known remaining gap: `ResponseInterface::__construct()` declares 3 params vs `AbstractResponse`'s 4 — PHP does not enforce constructor signatures, so it is latent, not breaking-in-practice; see the Jira issue before "fixing" it.)
