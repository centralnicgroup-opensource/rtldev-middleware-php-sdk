...
`@semantic-release/git` commits the version bump, `HISTORY.md` and the `VERSION` constant in `src/AbstractClient.php` and pushes them straight to `master`. That push cannot go through a pull request — it happens after the version has been decided — so branch protection has to let exactly that one identity past it.
