...
- **The lint workflow is merge-blocking, and the `name:` is what makes it so.** `lint.yml`'s `ci-success` job carries `name: "Lint: completed"`, so _that_ is the check-run name it publishes; `test.yml` has an identically-keyed `ci-success` job with no `name:`, so it publishes the bare context `ci-success`. `main` protection requires **both** contexts, so a prettier (or phpcs, or psalm) failure blocks the merge. Until RSRMID-2929 only `test.yml`'s bare `ci-success` was required and the whole lint workflow was advisory. Two things follow: never drop or rename `lint.yml`'s `name:` — that silently orphans the required context and the gate goes advisory again with nothing turning red; and a startup failure of the reusable call now surfaces as a permanently _pending_ required check rather than an invisible pass, because it produces zero check runs.
...
## Release push authentication (RSRMID-2994)

`@semantic-release/git` commits the version bump, `HISTORY.md` and the `VERSION` constant in `src/AbstractClient.php` and pushes them straight to `main`. That push cannot go through a pull request — it happens after the version has been decided — so branch protection has to let exactly that one identity past it.

- **The identity is a deploy key**, not a token. Its private half is the repository secret `RTLDEV_MW_CI_SSH_KEY`; the shared `php-sdk-release.yml` writes it to `~/.ssh/id_ed25519` before running semantic-release. The `default-branch-protection` ruleset names `DeployKey` as its only bypass actor. A deploy key reaches one repository, so a leak costs one repository — where the previous `RTLDEV_MW_CI_TOKEN` was a personal account's PAT with admin over all of them.
- **`repositoryUrl` in `.releaserc.json` is load-bearing and must stay in SSH form.** semantic-release's `lib/get-git-auth-url.js` expands shortcut URLs to HTTPS _before_ it ever tries SSH, and `package.json`'s `"repository": "github:centralnicgroup-opensource/rtldev-middleware-php-sdk.git"` is a shortcut. Without the explicit `git@github.com:…` override SSH is never attempted, the push falls back to HTTPS, and the ruleset rejects it.
- **`GITHUB_TOKEN` stays in the release step.** `@semantic-release/github` needs it to create the GitHub release, and `.github/scripts/publish-docs.sh` needs it to force-push `gh-pages`. Only the _git_ push moved to SSH.
- **The `gh-pages` push deliberately did not move.** The ruleset targets `~DEFAULT_BRANCH`, so a push to `gh-pages` is not subject to it. Do not "tidy" that push onto the deploy key — it would widen what the key is used for without removing a restriction.
- **A failed SSH push is silent, but its consequence is loud.** If SSH auth fails, semantic-release falls back to HTTPS with `GITHUB_TOKEN` without re-verifying. That fallback push is then rejected by the ruleset, so the release fails visibly rather than pushing under the wrong identity. To see which path was taken, set `DEBUG: semantic-release:get-git-auth-url` on the release step and look for `SSH key auth successful.`

Because `DeployKey` is a bypass actor as a _class_ — the API requires `actor_id: null` — every write-enabled deploy key on this repository can bypass the ruleset. There must be exactly one: check with `gh repo deploy-key list -R centralnicgroup-opensource/rtldev-middleware-php-sdk`.
